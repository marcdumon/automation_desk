...
def test_new_links_only_and_the_first_read_rule(web: dict) -> None:
    ...
    got = collect(NOW - timedelta(days=1), NOW, httpx.Client(), allow_browser=False)
    assert [a.link for a in got.articles] == ['https://krant.be/a/1', 'https://krant.be/a/2'], (
        'first read of a feed: last 24 hours and undated items; the 3-day-old item is skipped')
    assert store.known_links(['https://blog.org/p/1']) == {'https://blog.org/p/1'}, 'front page first read: recorded as seen'
    assert len(got.articles[0].text.split()) == 500, 'at most 500 words go to the model'
    assert [s.last_result for s in store.sources()] == ['2 new', 'first read: 1 link recorded, new ones from the next digest']
    assert feed and front
    store.mark_seen(feed, [a.link for a in got.articles])
    again = collect(NOW, NOW + timedelta(hours=1), httpx.Client(), allow_browser=False)
    assert [a.link for a in again.articles] == [], 'an undated item is picked up once, never repeated'
...
def test_blocked_site_uses_the_teaser_at_once_when_unattended(web: dict) -> None:
    ...
    assert (article.text, article.blocked) == ('Teaser 9', True) and web['browser'] == []
