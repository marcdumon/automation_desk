...
def item(n: int, subject: str, batch: int, text_words: int = 10, same: tuple[int, ...] = ()) -> Summarised:
    """A summarised article number n."""
    article = Article(f'https://s.be/{n}', n, f'Site {n}', f'Titel {n}', datetime(2026, 9, 24, tzinfo=UTC), '',
                      'w ' * text_words)
    return Summarised(article, f'Samenvatting {n}', subject, '', False, '', batch, frozenset(f'https://s.be/{m}' for m in same))


def test_groups_across_batches_are_checked_and_merged(monkeypatch: pytest.MonkeyPatch) -> None:
    items = [item(1, 'Politics BE', 0, same=(2,)), item(2, 'Politics BE', 0, same=(1,)), item(3, 'Other', 1, text_words=50),
             item(4, 'AI', 1)]
    monkeypatch.setattr(module, 'ask', lambda *a, **k: MergeGroups(groups=[[1, 3], [2, 99], [4, 4]]))
...
def test_merger_failure_keeps_the_batch_merges(monkeypatch: pytest.MonkeyPatch) -> None:
    def down(*a: object, **k: object) -> MergeGroups:
        """The merger call fails every time."""
        raise LLMError('down')

    monkeypatch.setattr(module, 'ask', down)
    stories, problems = merge_stories([item(1, 'AI', 0, same=(2,)), item(2, 'AI', 0, same=(1,)), item(3, 'AI', 1)])
    assert sorted(len(s.articles) for s in stories) == [1, 2]
    assert problems == ['Stories were not merged across batches (down); a story may appear twice.']
